...
// h. Low battery: measured before sleep only; one 3 s buzzer + LED alert per low event, then sleep.
...
// Keeps watchdog wake off so the chip doesnt wake itself and repeat pre-sleep alerts.
...
// Remembers that the user already got warned for the current low-battery event.
static bool railLowAlertLatched = false;
// Stores the most recent pre-sleep rail reading for debug and Serial messages.
static uint16_t lastRailCheckMv = 5000;
...
static void printRailDiagnostic() {
  ...
  Serial.print(F("Low-alert latch: "));
  Serial.println(railLowAlertLatched ? F("holding off repeat alerts") : F("ready"));
}

// Converts the measured rail voltage into 0 = OK, 1 = warning, 2 = critical.
static uint8_t classifyRailLevel(uint16_t railMv) {
  if (railMv < RAIL_CRIT_MV) return 2;
  if (railMv < RAIL_WARN_MV) return 1;
  return 0;
}
// Clears the one-shot low-battery latch only after the rail truly recovers.
static void updateRailAlertLatch(uint16_t railMv) {
  if (railLowAlertLatched && railMv >= RAIL_RECOVER_MV) {
    railLowAlertLatched = false;
    Serial.println(F("5 V rail recovered; low-battery alert re-armed."));
  }
}
...
static void enterSleepIfIdle() {
  ...
  if (lockState == STATE_LOCKING || lockState == STATE_UNLOCKING) 
    return;
  // Right before sleep: one rail check; low rail gets one 3 s alert until the rail recovers.
  {
    uint16_t rail = readFiveVRailMilliVoltsAveraged();
    lastRailCheckMv = rail;
    updateRailAlertLatch(rail);
    uint8_t preSleepLevel = classifyRailLevel(rail);

    // Plays the human-facing alert only once for the current low-battery event.
    if (preSleepLevel != 0 && !railLowAlertLatched) {
      Serial.println(F("Low supply before sleep: one 3 s alert, then sleep."));
      Serial.flush();
    ...
    // Skips repeat beeping if the same low battery is still present.
    else if (preSleepLevel != 0 && railLowAlertLatched) {
      Serial.println(F("Low supply already alerted; entering sleep quietly."));
      Serial.flush();
    }
  }
...
void setup() {
  ...
  Serial.println(F("  B        : print 5V rail and low-alert latch state"));
