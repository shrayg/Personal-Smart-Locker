...
/*
  ...
  Milestone 4d (merged into this final sketch):
    Servo power gating via D11 (PMOS + NPN driver) so servo rail is off when idle/sleeping.
*/
...
const uint8_t SERVO_PWR_PIN = 11;
...
void setup() {
  ...
  /* Startup alignment pulse, then cut servo rail at idle. */
  servoPowerOn();
  servoAttach();
  setServoPulse(RESET);
  delay(250);
